...
/**
 * @brief Address-routed I2C slave backend for the QUPv3 model.
 *
...
#include <ports/i2c-tlm-command.h>
...
class i2c_multi_slave_backend : public sc_core::sc_module
{
    ...
    struct slave {
        std::unique_ptr<socket_type> socket; // Connects this slave to the QUP I2C master.
...
public:
    ...
    void b_transport(tlm::tlm_generic_payload& txn, sc_core::sc_time&)
    {
        ...
        const bool is_combined = command == gs::i2c_write_read_command;
        gs::i2c_offset_extension* offset_extension = nullptr;
        txn.get_extension(offset_extension);
        const bool uses_offset = offset_extension != nullptr && offset_extension->uses_offset;
        ...
        // value write stores its byte at offset zero. The QUP extension
        ...
        case gs::i2c_write_read_command:
            write(slave_address, i2c_slave, data, write_len);
